...
public class CustomUserTaskJsonConverter extends UserTaskJsonConverter {
    ...
    protected FlowElement convertJsonToElement(JsonNode elementNode, JsonNode modelNode, Map<String, JsonNode> shapeMap) {
       ...
       usertaskassignment:
         单节点：{"assignment":{"assignee":"eoco","assigneType":1,"assigneTemp":"普通角色1","candidateUsers":[{"value":""}]}}
         抢占式节点：{"assignment":{"candidateUsers":[{"value":"eoco,Lily"}],"assignee":""}}
         会签节点：{"assignment":{"assignee":"${list}"}}
      */
       /* System.out.println("开始：");
        System.out.println(usertaskassignment);
        JsonNode assignment = usertaskassignment.findValue("assignment");
        JsonNode assigneType = assignment.findValue("assigneType");
        if(assigneType!=null && "1".equals(assigneType.asText())){
...
*/
        ...
        /*if(true){
            List<ExtensionElement> list=new ArrayList<>();
            ExtensionElement extension=new ExtensionElement();
            extension.setName("mytype");
            extension.setNamespace("mytype");
            extension.setNamespacePrefix("activiti");
            extension.setElementText("1");
            list.add(extension);
            extensionElements.put("jh12",list);
            task.setExtensionElements(extensionElements);
        }*/
        //System.out.println("userAssigne:"+this.getPropertyValueAsString("usertaskassignment", elementNode));
        //解析新增属性的业务逻辑
