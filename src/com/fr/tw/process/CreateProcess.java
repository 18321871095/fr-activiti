...
public class CreateProcess {
        ...
        try {
            ...
            if(modelData.getTenantId().equals(currentUserNameFromRequestCookie) || ProcessUtils.isAdmin(currentUserNameFromRequestCookie)) {
              ...
              /*  String proName=modelData.getMetaInfo().split("\"name\":")[1].
                ...
                //这里需要转换办理人（目前设置了办理人可以选择角色）
                Map<String, Object> map = updateByte(modelEditorSource);
                if("err".equals(map.get("message"))){
                    jr.setMsg("fail");
                    jr.setResult("数据转换失败:"+map.get("data"));
                }else{
                    JsonNode modelNode = (JsonNode) new ObjectMapper().readTree((byte[]) map.get("data"));
    ...
    //因为新增加了办理人选择角色，在转换成xml时，把assignee与assigneTemp对换
    public Map<String,Object> updateByte(byte[] modelEditorSource) throws Exception {
        //
                /* JsonNode usertaskassignment = this.getProperty("usertaskassignment", elementNode);
                   usertaskassignment:
                     单节点：{"assignment":{"assignee":"普通角色1","assigneType":1,"assigneTemp":"eoco","candidateUsers":[{"value":""}]}}
                     抢占式节点：{"assignment":{"candidateUsers":[{"value":"eoco,Lily"}],"assignee":""}}
                     会签节点：{"assignment":{"assignee":"${list}"}}
                 */
        Map<String,Object> map=new HashMap<>();
        try{
           // System.out.println(new String(modelEditorSource,"utf-8"));

            JSONObject source=new JSONObject(new String(modelEditorSource,"utf-8"));
            JSONArray childShapes = source.getJSONArray("childShapes");
            if(childShapes.length()==0){
                map.put("message","success");
                map.put("data",modelEditorSource);
            }else{
                for(int i=0;i<childShapes.length();i++){

                    String stencil = childShapes.getJSONObject(i).getJSONObject("stencil").getString("id");
                    if("UserTask".equals(stencil)){
                       ...
                       // System.out.println("formkeydefinition:"+formkeydefinition);
                        ...
                        //办理人
                            ...
                            if("1".equals(assigneType) && !"".equals(assigneeValue)){
                                ...
                                String assigneTemp = assignment.getString("assigneTemp");
                                assignment.put("assignee",assigneTemp);
                                assignment.put("assigneTemp",assignee);
                            }
                            String preemptiveType = assignment.getString("preemptiveType");
                            Object object_candidateUsers = assignment.get("candidateUsers");
                            String value="";
                            JSONArray candidateUsers=null;
                            if(object_candidateUsers!=null){
                                candidateUsers = assignment.getJSONArray("candidateUsers");
                                value  = candidateUsers.getJSONObject(0).getString("value");
                            }
                            if("1".equals(preemptiveType) && !"".equals(value)){
                                String preemptiveTemp = assignment.getString("preemptiveTemp");
                                JSONArray groupRoles = new JSONArray(preemptiveTemp);
                                String[] split = value.split(",");
                                String preemptiveAssigne="";
                                for(String str : split){
                                    for(int m=0;m<groupRoles.length();m++){
                                        if(str.equals(groupRoles.getJSONObject(m).getString("name"))){
                                            //设置抢占式任务办理人
                                            String user = groupRoles.getJSONObject(m).getString("user");
                                            preemptiveAssigne+=user.replace("[","").
                                                    replace("]","").replace("\"","")+",";

                                        }
                                    }
                                }//for
                                candidateUsers.getJSONObject(0).put("value",
                                        preemptiveAssigne.length()==0?"":preemptiveAssigne.substring(0,preemptiveAssigne.length()-1));
                            }

                            String huiqianType = properties.getString("huiqianType");
                            if("1".equals(huiqianType) && !"".equals(huiqianType)){
                                String huiqian = properties.getString("huiqian");
                                String huiqianTemp = properties.getString("huiqianTemp");
                                JSONArray huiqianUser = new JSONArray(huiqianTemp);
                                String[] split = huiqian.split(",");
                                String huiqianAssigne="";
                                for(String str : split){
                                    for(int m=0;m<huiqianUser.length();m++){
                                        if(str.equals(huiqianUser.getJSONObject(m).getString("name"))){
                                            //设置抢占式任务办理人
                                            String user = huiqianUser.getJSONObject(m).getString("user");
                                            huiqianAssigne+=user.replace("[","").
                                                    replace("]","").replace("\"","")+",";

                                        }
                                    }
                                }//for
                               // huiqianAssigne Billy, sunlin, Tom中间会有空格
                                properties.put("huiqian",
                                        huiqianAssigne.length()==0?"":huiqianAssigne.substring(0,huiqianAssigne.length()-1));

                               // System.out.println( huiqianAssigne.length()==0?"":huiqianAssigne.substring(0,huiqianAssigne.length()-1));
                             /*   System.out.println(huiqian);
                                System.out.println(huiqianTemp);*/
                            }
                      }
                }
                map.put("message","success");
                map.put("data",source.toString().getBytes("utf-8"));
            }
            return map;
        }catch (Exception e){
            map.put("message","err");
     ...
     * 辅助功能页面初始化
     */
    @RequestMapping(value = "/initAssistFuntion")
    @ResponseBody
    public JSONResult initAssistFuntion(HttpServletRequest request) throws Exception {
        JSONResult jr = new JSONResult();
        try {
            List<Map<String,String>> list=new ArrayList<>();
            Map<String, String> attachmentMap= ProcessUtils.getAttachmentPath(request);
            if("2".equals(attachmentMap.get("type").toString())){
                jr.setMsg("fail");
                jr.setResult(attachmentMap.get("message"));
                return  jr;
            }else{
                attachmentMap.put("id","attachment");
                list.add(attachmentMap);
                jr.setMsg("success");
                jr.setResult(list);
                return  jr;
            }
     ...
     * 辅助功能页面初始化获取本地文件夹路径
     */
    @RequestMapping(value = "/getLocalFiles")
    @ResponseBody
    public JSONResult getLocalFiles(HttpServletRequest request,String path) throws Exception {
        JSONResult jr = new JSONResult();
        List<Map<String,String>> list=new ArrayList<>();
        String message="";
        try {
     ...
     * 辅助功能页面初始化保存自定义路径
     */
    @RequestMapping(value = "/confirmCustomPath")
    @ResponseBody
    public JSONResult confirmCustomPath(HttpServletRequest request,String path,String type) throws Exception {
        JSONResult jr = new JSONResult();
        try {
            if("1".equals(type)){
                if("".equals(path) || path==null){
                    ...
                    ProcessUtils.confirmAttachmentPath(request,path);
                    jr.setMsg("success");
                    jr.setResult("success");
                }
            }else{
                ProcessUtils.confirmAttachmentPath(request,"");
                jr.setMsg("success");
                jr.setResult("success");
            }
            return jr;
