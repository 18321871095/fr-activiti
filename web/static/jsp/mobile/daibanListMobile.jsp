...
<div class="mui-content">
    <div id="slider" class="mui-slider mui-fullscreen">
        ...
        <div id="search" style="width: 100%;height: 45px;background: #fff;">
            <input type="text" id="proname" placeholder="流程名称" style="width: 35%;height: 30px;margin-top: 5px;margin-left: 5px;" />
            <input type="text" id="proinstanceid" placeholder="流程编号"style="width: 35%;height: 30px;margin-top: 5px;" />
            <button id="daibanBtn" style="margin-top: 5px;height: 30px;background: #4cd964;padding: 0px 12px;color: #fff;">搜索</button>
        </div>

        <div id="mydiv" class="mui-slider-group toDo" style="top: 85px;" >
...
<script>
    ...
    (function($) {
        ...
        $.ready(function() {
            var daibanCount=1;
            var daiban_self;
            var daiban_search=false;
            var proname="";
            var proinstanceid="";
            var daiban_empty=false;
            var daiban_load=false;
            ...
            //待办搜索
            $("#slider").on("tap","#daibanBtn",function (){
                proname =jQuery("#proname").val();
                proinstanceid=jQuery("#proinstanceid").val();
                daiban_search=true;
                daiban_empty=true;
                daibanCount=1;
                if(daiban_load){
                    daiban_self.refresh(true);
                }
                getDaibanList(daibanCount,daiban_self,"2",proname,proinstanceid);
            });
            ...
            function initdaiban(){
                $("#daibanrefresh").pullToRefresh({
                    up: {
                        ...
                        callback: function() {
                            daiban_self = this;
                            var type=daiban_search ? "2" : "1";
                            getDaibanList(daibanCount,daiban_self,type,proname,proinstanceid);
                        }
                    }
                });
            }
           ...
           //获取待办列表
            function getDaibanList(page,self,type,proname,proinstanceid){
                $.showLoading("","div");
                $.ajax("${ctx}/mobile/selectTask",{
                    type: "POST",
                    data:{num:page,type:type,proName:proname,proInstanceId:proinstanceid},
                    dataType: "json",
                    success: function (data) {
                        $.hideLoading();
                        if(data.msg==='success'){
                            ...
                            daiban_load=daibanCount>=data.yeshu;
                            daibanCount++;
                        }else {
