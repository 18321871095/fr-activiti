...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/static/css/proDeployInfo.css?v=1">
...
<div class="tab">
    ...
    <div class="bd">
        ...
        <div id="fuzhu" class="bd-son">
            <div>
                <div class="layui-form">
                    <div class="layui-form-item">
                        <label style="width: 150px;" class="layui-form-label">附件路径类型</label>
                        <div class="layui-input-block">
                            <input lay-filter="attachmentFilter" type="radio" id="moren" name="attachmentType" value="0" title="默认路径">
                            <input lay-filter="attachmentFilter" type="radio" id="zidingyi" name="attachmentType" value="1" title="自定义路径">
                        </div>
                    </div>
                    <div>
                        <div  class="layui-form" style="width: 50%;display: inline-block;margin-left: 80px;">
                            <input   type="text" id="attachmentPath" readonly="readonly"   class="layui-input">
                        </div>
                        <div  class="layui-form" style="width: 150px;display: inline-block;">
                            <button  id="selectPath" class="layui-btn">选择路径</button>
                            <button  id="confirmDefault" class="layui-btn">保存默认路径</button>
                        </div>
                        <div id="fileDiv" style="border: 1px solid #333;width: 50%;height: 60%;margin-left: 80px;display: none;">
                            <div style="width: 100%;height: 90%;overflow-y: auto;">
                                <ul id="root" style="line-height: 35px;margin-left: 5px;">
                                </ul>
                            </div>
                            <div>
                                <button id="cancel" class="layui-btn layui-btn-warm" style="float: right;margin-top: 3px;line-height: 27px;height: 30px;margin-right: 5px;">取消</button>
                                <button id="confirm" class="layui-btn" style="float: right;margin-top: 3px;line-height: 27px;height: 30px;margin-right: 10px;">保存</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    var attachmentDefaultPath="";
    var attachmentCustomPath="";
    $(document).ready(function () {
        layui.use(['layer','form'], function() {
            deploylayer  = layui.layer;
            deployform=layui.form;
            initMoXing(1,true);
            ...
            $(document).on("click","${'[name=\'chakan\']'}",function(){
                ...
                window.parent.FS.tabPane.addItem({title:"流程图查看"+modelid, src:
                        "${ctx}/modeler.html?modelId="+modelid+"&modelerStatus=false&param="+myid});
            });
            ...
            $(document).on("click","${'[name=\'quedingquanxian\']'}",function(){
                ...
                if($("#quanxianname").val()==''){
                    ...
                    $.post("${ctx}/processInfo/baocunAuthority",{
                        procdefid:mythis.attr("id"),name:$("#quanxianname").val(),oldName:reserveAuthorityName
                    },function (data) {
                        ...
                        if(data.msg=='success'){
                            ...
                            deploylayer.msg("该流程还没有设置权限",{offset:'200px'});
            ...
            $(document).on("click","${'[name=\'daoruquanxian\']'}",function(){
                var procdefid=$(this).parents("tr").find("input[type='hidden']").eq(0).val();
                deploylayer.confirm('导入权限会覆盖已有的权限，是否继续?', {icon: 3, title:'提示',offset:'150px'}, function(index){
            ...
            //监听附件选择
            deployform.on('radio(attachmentFilter)', function(data){
               if(data.value=='0'){
                   $("#attachmentPath").val(attachmentDefaultPath);
                   $("#selectPath").hide();
                   $("#confirmDefault").show();
               }else{
                   $("#attachmentPath").val(attachmentCustomPath);
                   //$("#selectPath").removeClass("layui-btn-disabled");
                   $("#selectPath").show();
                   $("#confirmDefault").hide();
               }
                $("#fileDiv").hide();
            ...
            $(document).on("click","${'.myspantitle'}",function(){
                ...
                $("#attachmentPath").val(path.replace(/\\/g,"/"))
               // console.log(path)
               // console.log(path.replace(/\\/g,"/"))
            });
            //保存自定义路径
            $("#confirm").click(function () {
                var path= $("#attachmentPath").val();
                if(path==''){
                    deploylayer.msg("请选择路径",{offset:'200px'});
                }else{
                    var index=deploylayer.load(2,{offset:'46%'});
                    $.ajax({
                        type: "POST",
                        data:{path:path,type:'1'},
                        dataType: "json",
                        async:false,
                        url: "${ctx}/processDiagram/confirmCustomPath",
                        success: function (data) {
                            if(data.msg=='success'){
                                ...
                                deploylayer.alert(data.result,{offset:'200px'});
                            }else{
                                deploylayer.alert("保存失败",{offset:'200px'});
                            }
                        },
    ...
    function initFuZhu() {
        ...
        $.ajax({
            ...
            dataType: "json",
            async:false,
            url: "${ctx}/processDiagram/initAssistFuntion",
            success: function (data) {
                deploylayer.close(index);
                if(data.msg==='success') {
                    for(var i=0;i<data.result.length;i++){
                        //attachment
                        if(data.result[i].id=='attachment'){
                            attachmentDefaultPath=data.result[i].defaultPath;
                            attachmentCustomPath=data.result[i].customPath;
                            if(data.result[i].type=='0'){
                                $("#moren").prop("checked",true);
                                $("#attachmentPath").val(data.result[i].defaultPath);
                                $("#confirmDefault").show();
                                $("#selectPath").hide();
                               // $("#selectPath").addClass("layui-btn-disabled");
                            }else{
                                $("#zidingyi").prop("checked",true);
                                $("#attachmentPath").val(data.result[i].customPath);
                                $("#confirmDefault").hide();
                                $("#selectPath").show();
                                //$("#selectPath").removeClass("layui-btn-disabled");
                            }
                    ...
                    window.location.href="${ctx}/static/jsp/message.jsp?message="+encodeURI("辅助功能页面初始化失败："+data.result);
                }
            },
            error: function (e, jqxhr, settings, exception) {
                deploylayer.close(index);
    ...
    function getFiles(path,objappend,marginLeft,currentobj) {
        ...
        $.ajax({
            ...
            dataType: "json",
            async:false,
            url: "${ctx}/processDiagram/getLocalFiles",
            success: function (data) {
                deploylayer.close(index);
                if(data.msg=='success'){
                    if(path==''){
