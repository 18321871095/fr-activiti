...
<head>
    ...
    <link href="${ctx}/static/css/daibanTask.css?v=1" rel="stylesheet">
</head>
        ...
        <div class="tab">
            ...
            <div class="bd">
                <div class="bd-son" style="display: block;" id="daiban">
                    ...
                    <div  class="layui-form" style="width: 150px;display: inline-block;">
                        <input   type="text" id="startPeople"  placeholder="发起人" class="layui-input">
                    </div>
                    <div  class="layui-form" style="width: 150px;display: inline-block;">
                    ...
                    <div  class="layui-form" style="width: 150px;display: inline-block;">
                        <input  type="text" id="proInstanceId"  placeholder="流程实例编号" class="layui-input">
                    </div>
                    <div class="layui-form" style="width: 200px;display: inline-block;">
...
$(function () {
    ...
    layui.use(['laypage', 'layer','element','table','laydate'], function() {
       ...
       var  daiban_laydate = layui.laydate;
        ...
        $(document).on("click","${'[name=\'banliTask\']'}",function(){
            var taskid=$(this).parents("tr").find("input[type='hidden']").eq(2).val();
            ...
            window.parent.FS.tabPane.addItem({title:proname+":"+proInstanceId,src:
        ...
        $(document).on("click","${'[name=\'zhankai_dep\']'}",function(){
            var dep = $(this).parents("tr").find("input[type='hidden']").eq(3).val();
            alert(dep)
        });
    ...
    function selDaiBanTask() {
        ...
        var time=$("#time1").val();
        var proInstanceId=$.trim($("#proInstanceId").val());
        var dep=$("#dep").val();
        // alert(name+","+depName+","+time.split("-")[0]+","+parseInt(time.split("-")[1]));
        seldaiban(1,true,proName,startPeople,time,proInstanceId,dep);
    }
    function seldaiban(num,flag,proName,startPeople,time,proInstanceId,dep) {
        var index=daiban_layer.load(2,{offset:['200px','46%']});
        $.ajax({
            type: "POST",
            data:{userName:userName,num:num,proName:proName,startPeople:startPeople,time:time,proInstanceId:proInstanceId,
                depName:dep},
            dataType: "json",
                ...
                if(data.msg==='success'){
                    ...
                    if(flag) {
                        new myPagination({
                            ...
                            getPage: function (page) {
                                //获取当前页数
                                seldaiban(page,false,proName,startPeople,time,proInstanceId,dep);
                            }
                        });
                    }
                    linksList_daiban(data.result);
            ...
            if(datas.length==0){
                ...
                for(var i=0;i<datas.length;i++){
                    $("#daibanTbody").append("<tr>" +
                        ...
                        "<input type='hidden' value="+datas[i].taskId+" />"+
