            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        //console.log("获取model：")
                       // console.log(data)
                       /* var mydata=data.model.childShapes;
                        for(var i=0;i<mydata.length;i++){
                            if(mydata[i].stencil.id=='UserTask'){
                                if(mydata[i].properties.usertaskassignment!=''&&mydata[i].properties.usertaskassignment.assignment!=undefined){
                                    if(mydata[i].properties.usertaskassignment.assignment.assigneType=='1'){
                                        var temp= mydata[i].properties.usertaskassignment.assignment.assignee;
                                        mydata[i].properties.usertaskassignment.assignment.assignee=
                                            mydata[i].properties.usertaskassignment.assignment.assigneTemp;
                                        mydata[i].properties.usertaskassignment.assignment.assigneTemp=temp;
                                    }
                                }
                            }
                        }*/
                       // console.log("获取转换model：")
                       // console.log(data)
